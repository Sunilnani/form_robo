...
class DroneConnectSuccessMessage extends StatefulWidget {
  final String title;
  ...
  const DroneConnectSuccessMessage({Key? key, required this.title, this.description,this.isConnected,
    this.nextPage, this.image, this.messageType,this.imageHeight, this.btnTitle}) : super(key: key);

  @override
  State<DroneConnectSuccessMessage> createState() => _DroneConnectSuccessMessageState();
}

class _DroneConnectSuccessMessageState extends State<DroneConnectSuccessMessage> {

  void splash(){
    Future.delayed(Duration(seconds: 2),(){
      NavigationService().navigatePage(FlyScreen(isConnect: true,),replace: true);
